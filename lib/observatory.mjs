...
export function buildObservatorySnapshot({ baseUrl, observedAt, documents }) {
  ...
  const versions = [openapi.info.version, agent.version, config.version, skills.skills?.[0]?.version];
  const checks = [
    check(
    ...
    check(
      ...
      `OpenAPI, agent manifest, config and skills index report ${versions.join(" / ")}`,
    ),
    check(
...
export function verifyObservatoryArtifact(manifest, artifact) {
  ...
  if (artifact.snapshot.documents.length !== OBSERVATORY_PATHS.length) errors.push("unexpected discovery-surface count");
